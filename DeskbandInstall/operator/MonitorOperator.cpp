...
bool CMonitorOperator::loadDLL()
{
	...
	if (!m_hDll)
	{
		::MessageBox(NULL, L"º”‘ÿDLL ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		return false;
	}
	...
	if (!m_regsterFunc)
	{
		::MessageBox(NULL, L"DllRegisterServer º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}

	if (!m_unregsterFunc)
	{
		::MessageBox(NULL, L"DllUnregisterServer º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}

	if (!m_isregsterFunc)
	{
		::MessageBox(NULL, L"DLLIsRegster º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}

	if (!m_showFunc)
	{
		::MessageBox(NULL, L"DllShowMonitor º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}

	if (!m_isShowFunc)
	{
		::MessageBox(NULL, L"DllIsShow º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}

	if (!m_enableLogFunc)
	{
		::MessageBox(NULL, L"DllEnableLog º”‘ÿ ß∞‹£°", L"¥ÌŒÛ", MB_OK);
		goto load_ret;
	}
...
void CMonitorOperator::help()
{
	...
	printf(sHelp.c_str());
	MessageBoxA(NULL, sHelp.c_str(), "Ã· æ(Tips)", MB_OK | MB_ICONINFORMATION);
}